...
def write_search_index(out: pathlib.Path, postings: dict[str, list[int]]) -> None:
    ...
    Whole, the index is 17 MB, 6.5 MB of it over the wire -- too much to hand a
    phone before it can search at all. Split by the first two characters of the
    word, a query fetches one file (a few KB for most, 216 KB gzipped for
...
<p>Words in quotes are a phrase, and the index cannot answer for those: it says
which threads hold a word, not where in the thread the word falls. So the
threads that hold every word of the phrase are looked up first, and pressing
Enter reads those pages to keep the ones that say the words in that order. It
is a page fetch each, so it stops after sixty and says that it did.</p>
...
def render_search() -> str:
    """The search page: a box, a status line, and the script that fills them.

    The script itself is ``static/search.js``, copied to the site as it stands.
    What the index was built with travels on the form, so the page and the
    files it reads cannot drift apart.
    """
    body = f"""<nav class="crumbs"><a href="../">Archive</a></nav>
<h1>Search</h1>
<div class="help">
<p>Every word of every message, with thread titles and author names. A word
matches from the start; type at least {WORD_MIN} characters. Several words
narrow the search.</p>
<p>Quotes ask for the wording. <code>"the vampire is burned"</code> finds the
threads holding all four words as you type, and Enter reads those threads to
keep the ones that say them in that order.</p>
</div>
<form id="f" onsubmit="return false" data-min="{WORD_MIN}" data-max="{WORD_MAX}"
  data-cap="{PREFIX_CAP}"><input id="q" type="search"
...
def main() -> int:
    ...
    for count, path in enumerate(paths, 1):
        ...
        #: What a search result shows. Author names are not here: the word
        #: index holds them, which is how a search for one finds the thread,
        #: and carrying them again would double the file every search fetches.
        index.append(
            [
                thread.id,
                thread.title,
    ...
    for asset in ("site.css", "search.js"):
        shutil.copyfile(here / "static" / asset, out / asset)
    write(out / ".nojekyll", "")
