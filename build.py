...
def render_search() -> str:
    ...
    whole index. Words in quotes are asked of the threads themselves: the index
    files hold no word positions, so the wording is settled by reading the
    candidate pages, which is why that step waits for Enter.
    """
...
<script>
(function () {
  var MIN = __MIN__, MAX = __MAX__, CAP = __CAP__;
  ...
  form.addEventListener('submit', function (event) {
    ...
    body = (
        body.replace("__MIN__", str(WORD_MIN))
        .replace("__MAX__", str(WORD_MAX))
        .replace("__CAP__", str(PREFIX_CAP))
    )
